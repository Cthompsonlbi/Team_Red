...
The results from the machine learning models suggest that they support this analysis, given the extracted feature importances and high accuracy scores. Age, monthly income, and duration of pitch were the most heavily weighted features when determining if a customer purchased the product or not. Ultimately, there is a black-box element to consider when using more advanced machine learning models, though we at least know the models have high accuracy, which, itself, is meaningful. High accuracy tells us that the provided information is enough to confidently determine outcomes. When paired with analyses on Tableau, or when visualizing our five market segments with clustering models, we can further trust these models. Our team feels these models adequately meet our client's expectations, and can be leveraged extensively to increase conversion rates while reducing sunk costs related to agent time spent pitching the products.
